...
func BuildUserPrompt(data entity.PromptData) string {
	...
	r := strings.NewReplacer(
		...
		// --- 账户字段 ---
		"{return_pct}", fmt.Sprintf("%.2f", data.Account.ReturnPct),
		"{sharpe_ratio}", fmt.Sprintf("%.2f", data.Account.SharpeRatio),
		"{cash_available}", fmt.Sprintf("%.2f", data.Account.CashAvailable),
		"{account_value}", fmt.Sprintf("%.2f", data.Account.AccountValue),
